...
public class CreateVideoPost extends AppCompatActivity {
    ...
    private void torealtime(){
        ...
        FirebaseDatabase.getInstance().getReference().child("videoposts").child(mycurrenttime2).
        ...
        FirebaseDatabase.getInstance().getReference().child(FirebaseAuth.getInstance().getUid()+"videoposts").child(mycurrenttime2).
                setValue(u).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
